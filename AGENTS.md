...
|---|---|---|---|---|
| **草单** | A原始 / C1问题 / C2确认 分类；客编 + 箱号提取；转发或转待确认 | `mailbots_next/core/extractors/draft.py`；旧 `mailbots/Draft_Forward_Robot.py` + `draft_pending.py` | 读 `yxo.db.records` | `草单转发规则稿_v3.1_全量验证修正.md`；**B「更新草单」已移除**（恒判 A，见 `mailbots_next/README.md` 技术债 X4） |
...
订舱记录会出现状态 = **「退舱」** 的行，表示这票业务已退。**记录本身要作为凭据长期留存**，它不属于可清理的垃圾数据。

**触发场景**：舱单导入 / Excel 导入 / 手工改行、去重、清理、迁移、状态改写、按班列整理 —— 任何会**读到或写到 `records` 表**的批量动作。
...
- 存在 **5 组**「同一数字段、一正常一退舱 / 双退舱」的客编，均为正常留痕，**应保留**：
  ...
  - `CQWLJT260923001`：id 1827 正常 / id 2433 退舱（⚠️ **这两条的「客户编码」是同一个字符串** —— 任何"按客户编码取单行"的写法（`fetchone()` 且无 `ORDER BY`）都会命中**不确定**的一条，命中退舱那条就是覆盖凭据）
- **代码侧已落地**：`manifest_engine.load_records()` 已在 SQL 层剔除退舱与软删
  （`WHERE COALESCE("状态",'')<>'退舱' AND COALESCE(is_deleted,0)=0`，见 `manifest_engine.py:463-468` 改动三注释）
  —— 即原「客编重复无法定位」的缺陷**已修复**。
- **待补充**：除「退舱」外是否还有其他只能留、不能动的终态，等毛骁洋给完整清单。（当前「状态」字段的候选取值只有 **正常 / 退舱 / 延期** 三个 —— `config.py:76`。）
...
> 🖥️ **换了一台机器怎么办**：上面那条路径是**洋的工作机**专用（服务器上没有 `Roke8x` 这个用户，照抄必失败）。在其他机器上**用该机自己的解释器**，但必须 ① 先自检 `-c "import sys; print(sys.executable)"` 记住它是哪个 ② **装依赖与跑测试必须是同一个** ③ 把自检结果写进报告，别只报"绿了"。**唯一性的要求是"同一台机器上只用同一个、且依赖装在它里面"，不是"全世界都用同一条路径"。**
