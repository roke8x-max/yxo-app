...
def mask_email(email: str) -> str:
    """敏感信息脱敏（全仓唯一实现）："maoxiaoyang@cqtransit.com" -> "ma***g@cqtransit.com"。

    放在模块级而非 EmailLogger 的私有方法：loud fail 路径（core/notify.py）也要用它，
    而跨模块调用私有方法一旦改名，就会从"静默失败"变成"崩在告警自身"。
    """
...
def mask_recipients(recipients) -> list:
    """收件人列表脱敏（全仓唯一实现）。"""
...
class EmailLogger:
    ...
    # 兼容保留：实现已提升为模块级 mask_email / mask_recipients（唯一实现在那里）。
    # 保留这两个方法名，是因为既有用例直接断言它们（tests/test_p1_coverage.py::test_mask_email_rule）。
    def _mask_email(self, email: str) -> str:
        return mask_email(email)

    def _mask_recipients(self, recipients: list) -> list:
        return mask_recipients(recipients)
    ...
    def log_email_received(self, message_id: str, folder: str, sender: str, subject: str, email_type: str = ""):
        masked_sender = mask_email(sender)
        self.info(
    ...
    def log_routing(self, message_id: str, row_idx: int, company: str, responsible_person: str, to_list: list, cc_list: list):
        self.info(
            f"Routing resolved | msg_id={message_id[:50]} | row={row_idx} | company={company} | responsible={mask_email(responsible_person)} | to={mask_recipients(to_list)} | cc={mask_recipients(cc_list)}"
        )
    ...
    def log_notify(self, message_id: str, notify_type: str, recipients: list, success: bool):
        masked = mask_recipients(recipients)
    ...
    def log_imap(self, account: str, folder: str, action: str, detail: str = ""):
        masked_account = mask_email(account)
