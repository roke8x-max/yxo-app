...
def _map_columns_text(header_row: List[str]) -> Dict[str, int]:
    """根据 CSV 文本表头映射列索引。

    第三道防线（文本/CSV 回退）专用。原本缺失导致 _parse_as_text 必然
    NameError，该路径从未生效。此处按 _map_columns 的写法对齐补齐。
    """
    col_map = {}
    keywords = {
        'container_no': ['箱号', 'container', '柜号', 'container_no'],
        'container_type': ['箱型', 'container_type', '柜型'],
        'chinese_wagon': ['车号', 'chinese_wagon', '铁路车号'],
        'destination': ['目的站', 'destination', '到站'],
        'departure_time': ['离站时间', 'departure_time', '发车时间'],
        'train_no': ['班列号', 'train_no', '列车号'],
        'waybill': ['运单号', 'waybill', '提单号'],
    }
    for key, aliases in keywords.items():
        for i, cell in enumerate(header_row):
            if any(alias.lower() in cell.lower() for alias in aliases):
                col_map[key] = i
                break
    return col_map
...
def _parse_row_text(row: List[str], col_map: Dict[str, int]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        'destination': _get_text_cell(row, col_map, 'destination'),
        'train_no': _get_text_cell(row, col_map, 'train_no'),
        'departure_time': _get_text_cell(row, col_map, 'departure_time'),
        'waybill': _get_text_cell(row, col_map, 'waybill'),
    }
