...
| X3 | 日志前缀与实际来源不符：`core/log.py` 的 `get_logger(name)` 返回**单例**并忽略传入的 `name` ⇒ 所有模块的日志都挂着**第一个调用者**的名字（例：`core/wecom_notify.py` 的报错会显示成 `core.dedup:`） | 排查时会**误判来源**（把人带到不相干的模块上），但不影响功能；修它要动日志单例的初始化方式（`log.py` 的 `_logger_instance`），属于会碰到**全仓**日志输出的改动 | **下批一并修复**（洋 2026-09-21 登记）。做法：把单例改成按 `name` 缓存的多实例，或至少让前缀反映真实调用模块；改完需复核既有日志相关用例不受影响 |
