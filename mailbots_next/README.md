...
> **`--once` 已实现**（2026-09-20）：**顺序跑一轮即退出** —— 12 条 Poller 逐个 `run_once`，不起线程、不循环、也不启动 sweeper / 每日汇总；用于部署自检与联调。
> ⚠️ 两个语义要点：① 它**不启动标已读 batcher** ⇒ 跑完**不会把邮件标为已读**（作为"无副作用自检"是有意的，别据此判断标已读链路）；② 退出码 0 = 轮次跑完，**不代表 SMTP 真的发出去了** —— 发信结果仍看日志与 `forward_log`。
> 冒烟仍建议配合 `MAILBOT_MODE=test` 短跑 + 日志确认。
...
1. **企微通知 loud fail（不再静默）**：`core/notify.py` 是单一咽喉 —— 客户端不可用
   （缺配置 / 构造失败）时启动期记一次 ERROR；每次发不出去记 ERROR（含通知类型与
   脱敏收件人）+ `notify_failed` 计数，返回 False（不重试、不抛异常、不阻断转发主链路）。
   运维看 `notify_failed` 计数与 ERROR 日志。仍依赖旧 `wecombot` 包（生产跑的是
   `D:\YXO_DATA\WeComBot` 另一份代码），HTTP 化见 C 组（待端点就绪后独立批次做）。
...
| C1 | 记录「负责公司」（`开票子公司名称`）为空时，该箱邮件**无法路由**（判 `no_route`、转不出去） | 空值来源是**网页端舱单导入**（`manifest_engine.apply_diff` 的导入通道本就不写这一列）+ 人工尚未指定；**机器人侧没有可用数据源去"猜"公司**，而猜错 = 把邮件发给错误的客户，比不发更糟（旧系统那套"空则补"读的也是库内值，本场景无数据可补） | 属**已确认可接受**的行为，不打算自动补齐。运维提示：遇到"某箱邮件一直不转"时**先查这条记录的「负责公司」是否为空**；人工在网页端表格指定该字段即可恢复转发 |
| T2 | 空邮箱每轮仍做全量 UID 搜索（无新邮件时也 SEARCH 一次） | 正确性优先：水位线本来就要每轮确认“有无新增”，一次轻量 SEARCH 是保活的一部分；当前 12 连接 × 30 秒负载可接受 | 观测到 IMAP 负载异常时再评估 UIDNEXT 轮询等优化 |
...
| X3 | 日志前缀与实际来源不符：`core/log.py` 的 `get_logger(name)` 返回**单例**并忽略传入的 `name` ⇒ 所有模块的日志都挂着**第一个调用者**的名字（例：`core/wecom_notify.py` 的报错会显示成 `core.dedup:`） | 排查时会**误判来源**（把人带到不相干的模块上），但不影响功能；修它要动日志单例的初始化方式（`log.py` 的 `_logger_instance`），属于会碰到**全仓**日志输出的改动（该单例已改名为 `_logger_instances`，本条已修，见右栏） | **已修（2026-09-21）**：改成按 `name` 缓存的多实例，同一 `name` 复用同一实例，前缀反映真实调用模块；格式/输出目标/级别不变 |
| X4 | 「草单更新/作废」（B 类）重新启用需要三项前置，缺一不可：① 台账（记录"机器人转发过什么"）② **内容指纹**（判断"和上一版是否真的不同"——注意草单文件名自带**秒级时间戳**，同票重发文件名必然不同，而 md5 单向可靠：相同⇒绝对没变，不同⇒可能只是重新导出，故需单独设计）③ **人工转发的可见性**（机器人不监控「已发送」文件夹 ⇒ 这是语义不成立的根因） | 现状 = 恒判 A（新草单），这是**有意保留的正确语义**（既定方向：宁可漏标，绝不误标），**不要当缺陷修** | 重新启用需单独立项，三项前置缺一不可 |
