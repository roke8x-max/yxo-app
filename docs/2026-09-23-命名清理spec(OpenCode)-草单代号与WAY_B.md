...
- 🔴 **版本表述收口：把 `mailbots_next/README.md:54` 的「Python 3.10+」改成「Python 3.11+」** —— 采纳 `StrEnum` 的**必要配套**（3.11+ 才有 `enum.StrEnum`）。
  - **统一写成「Python 3.11+」**（**不是** 3.13+）：3.11 是 `StrEnum` 的**最低**要求；而且 `scripts/deploy/DEPLOYMENT_MANUAL.md:47` **现在写的就是「Python 3.11+」** ⇒ 改成 3.11+ 正好与部署手册**一致**（这也正是"README 那句 3.10+ 已陈旧"的旁证）。
  - ⚠️ **顺手核对 `scripts/deploy/DEPLOYMENT_MANUAL.md:47` 必须保持「Python 3.11+」**（不许被改成 3.10，也不要顺手改成 3.13 —— 那是"实际解释器版本"，与"最低要求"是两件事；解释器路径的表述在 `AGENTS.md` / `WORKFLOW.md` 里另有，本批不动）。
  - 🔴 **改完必须全仓 grep 一次**，不要只改 README 那一行就收工 —— 判据见 §3.3 第 6 条（含**豁免清单**，我实测过全仓命中分布）。
...
6. 🔴 **版本表述收口**（改完必须全仓跑一遍；**期望值恒为 `0`**）：
   ```bash
   grep -rnI "3\.10" . --exclude-dir=.git --exclude-dir=tmp --exclude-dir=superpowers --exclude='*命名清理spec*'
   ```
   **改前实测 = 1 行**（`mailbots_next/README.md:54:- Python 3.10+`）；**改完必须 = 0 行**（跑完把原始输出贴进报告）。
   > 🚫 **不要试图"全仓 0 命中"** —— 那做不到，而且**为凑判据去改文档本身就是禁止的**。三处**必须豁免**、也已在命令里排除：
   > - **本 spec 自身**（`--exclude='*命名清理spec*'`）：它必须引用旧表述才能说明"原来写的是什么"。⚠️ **不要用"数 spec 有几行"当期望值** —— 那个数字**会随 spec 编辑而变**（我第一版就这么写错了：写完"2 行"之后自己又加了几处引用，期望值当场失效）。**豁免掉、期望 0**，才是不漂移的写法。
   > - `docs/superpowers/**`（`--exclude-dir=superpowers`）：8-26 的历史设计与计划，**不许改写历史**（里面写的是 `Python 3.13`，本不该命中，排除是防误伤）。
   > - `mailbots/tests/data/yxo_test.db`：SQLite **二进制**夹具，普通 grep 会**误报命中** ⇒ 命令必须带 **`-I`**（实测带 `-I` 后命中 0）。
   > ⇒ 若结果不为 0，看剩的是哪行：**还剩 `mailbots_next/README.md` 那行 = README 没改**（这条判据抓的就是它）；出现别的行 = 有遗漏的表述，一并改。
   > 顺带核对：`scripts/deploy/DEPLOYMENT_MANUAL.md:47` **仍是「Python 3.11+」**（与 README 改后一致；本批不要求改它）。
...
四、grep 验收（§2.3 第 2/3/4 条 + §3.3 第 1/4/5/6 条，贴原始输出）
五、偏差与请示
六、不确定 / 我没做的事
```
