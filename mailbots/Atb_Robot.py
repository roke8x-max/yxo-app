...
"""
ATB 邮件自动转发机器人 (v3)
基于 Dsk_Robot v3 架构，适配 ATB 邮件规则。新增：本地缓存优先加载 + 飞书 API 重试 + 转发后本地缓存清理。
...
def run_atb_robot():
    ...
    # 改造后：首次播种（飞书日志表 + 旧 JSON 历史指纹）后走本地主键点查 O(1)；90 天自动清理。
    if not TEST_MODE:
        dedup_store.seed_from_feishu(
            DEDUP_SOURCE, fs, TABLE_DSK_LOG, field_name="邮件唯一标识",
            extra_ids=load_local_dedup(), log=log
        )
        try:
            ...
            try:
                ...
                for box_no, extra_info in boxes:
                    ...
                    try:
                        ...
                        # 回写 ATB 时间戳到总表（2026-07-31 芙蕾雅修复：缓存命中时 box_record_id 为空，改现查总表兜底）
                        if not TEST_MODE:
                            _write_stamp(fs, TABLE_MAIN, box_record_id, box_no, "ATB")
...
# 根因：缓存命中运行时 box_record_id 被置空({})，导致原 706 行的
#   `if box_no in box_record_id` 恒为 False，ATB 时间戳几乎从不回写总表。
